...
export default function PublicOfferings({ onLoginClick }: PublicOfferingsProps) {
  ...
  const filterOfferings = () => {
    ...
    // Broadband-specific filters using simple text fields
    if (filters.mainCategory === 'Broadband' || filters.mainCategory === 'broadband') {
      // Filter by Connection Type
      if (broadbandFilters.connectionType !== 'all') {
        filtered = filtered.filter(offering => {
          return (offering as any).connectionType === broadbandFilters.connectionType;
        });
      }
      ...
      if (broadbandFilters.packageUsageType !== 'all') {
        filtered = filtered.filter(offering => {
          return (offering as any).packageType === broadbandFilters.packageUsageType;
        });
      }
      ...
      if (broadbandFilters.packageType !== 'all') {
        filtered = filtered.filter(offering => {
          return (offering as any).packageType === broadbandFilters.packageType;
        });
      }
      ...
      if (broadbandFilters.dataBundle !== 'all') {
        filtered = filtered.filter(offering => {
          return (offering as any).dataAllowance === broadbandFilters.dataBundle;
        });
      }
    }
  ...
  const getOfferingSpecs = (offering: ProductOffering) => {
    // Use simple text fields directly
    const connectionType = (offering as any).connectionType || 'Broadband';
    const packageType = (offering as any).packageType || 'Unlimited';
    const internetSpeed = (offering as any).internetSpeed || '300 Mbps';
    const dataAllowance = (offering as any).dataAllowance || 'Unlimited';
    const voice = (offering as any).calls || 'Unlimited Calls';
