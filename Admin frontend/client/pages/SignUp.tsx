...
export default function SignUp() {
  ...
  const validatePhone = (phone: string): { isValid: boolean; message: string } => {
    ...
    if (!digits.startsWith('07')) {
      return { isValid: false, message: 'Must start with 07' };
    }

    const thirdDigit = digits[2];
    const validOperators = ['0','1','2','5','6','7','8']; // 070,071,072,075,076,077,078
    if (!validOperators.includes(thirdDigit)) {
      return { isValid: false, message: 'Invalid mobile operator' };
    }
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="bg-white rounded-lg p-8 space-y-6 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Phone Number with Enhanced Validation */}
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                {phoneValidation.isTouched && (
                  <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
                    {phoneValidation.isValid ? <CheckCircle className="h-5 w-5 text-green-500" /> : <XCircle className="h-5 w-5 text-red-500" />}
                  </div>
                )}
              </div>
              {phoneValidation.isTouched && (
            ...
            {/* NIC Field */}
            <div className="space-y-2">
              <Label htmlFor="nic" className="text-gray-700 text-sm">Enter NIC *</Label>
              <div className="relative">
                <Input 
                  id="nic" 
                  type="text" 
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                />
                <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3 py-2 hover:bg-gray-100 text-gray-500" onClick={() => setShowPassword(!showPassword)} disabled={isLoading}>
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </Button>
              </div>
              {passwordStrength.isTouched && <p className={`text-xs ${passwordStrength.isValid ? 'text-green-600' : 'text-red-600'}`}>{passwordStrength.message}</p>}
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                />
                <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3 py-2 hover:bg-gray-100 text-gray-500" onClick={() => setShowConfirmPassword(!showConfirmPassword)} disabled={isLoading}>
                  {showConfirmPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </Button>
              </div>
              {passwordValidation.isTouched && <p className={`text-xs ${passwordValidation.isValid ? 'text-green-600' : 'text-red-600'}`}>{passwordValidation.message}</p>}
            </div>

            <div className="flex items-start space-x-2">
              <Checkbox id="agreeToTerms" checked={agreeToTerms} onCheckedChange={(checked) => setAgreeToTerms(checked as boolean)} className="border-gray-300 data-[state=checked]:bg-blue-500 data-[state=checked]:border-blue-500 mt-1" />
              <Label htmlFor="agreeToTerms" className="text-gray-700 text-sm leading-relaxed">I agree to the <Link to="/privacy-policy" className="text-blue-600 hover:underline font-medium">Terms of Service</Link> and <Link to="/privacy-policy" className="text-blue-600 hover:underline font-medium">Privacy Policy</Link></Label>
            </div>

            <div className="flex justify-center">
              <Button type="submit" className="bg-blue-600 hover:bg-blue-700 text-white font-medium px-8 py-3 rounded-lg h-12 w-full" disabled={isLoading}>
        ...
        <div className="text-center mt-4">
          <p className="text-white text-sm">Already have an account? <Link to="/login" className="text-blue-400 hover:underline font-medium">Sign in here</Link></p>
        </div>
      </div>
    </div>
  );
